defmodule PhoenixKitEntities.EntityData do
  ...
  * `:include_trashed` — when `true`, include trashed descendants
  ...
  # `Managed.renames_translated_slug?/2`, and `Managed.moves_data_record?/2`:
  # a save that resubmits the record's own unchanged slug, per-language
  # slug overrides, and entity_uuid (the ordinary case — a disabled
  ...
  # ACTUAL slug change, translated-slug change, or re-parenting pays for
  # the `Entities.get_entity/1` lookup (a `SELECT` plus `preload(:creator)`).
  # The policy itself (managed?, the `on_behalf_of` bypass) lives in
  ...
  defp validate_managed_slug(entity_data, attrs, opts) do
    if Managed.renames_data_slug?(entity_data, attrs) or
         Managed.renames_translated_slug?(entity_data, attrs) or
