defmodule PhoenixKitEntities.Test.LiveDatabaseGuard do
  ...
  Confirmed live (S014 recon, against `phoenix_kit_crm`'s copy of the same
  mechanism) that it does NOT close this one on its own:
  `SchemaOwnerGuard.check!/1` reads a database it has never seen —
  comment-less `schema_migrations` included — as `:ok`. A live dev
  database populated by ordinary `mix ecto.migrate` is exactly that
  shape: nothing has ever stamped it, so a marker check alone would wave
  it through. This guard instead refuses by NAME, against the specific
  databases this container must never let a test suite touch — the same
  check `pk-test` already makes, just checked here too.
  """
