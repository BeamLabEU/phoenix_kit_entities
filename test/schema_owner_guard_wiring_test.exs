defmodule PhoenixKitEntities.SchemaOwnerGuardWiringTest do
  ...
  test "a real boot through test_helper.exs stamps the owner marker", %{
    ...
    # A single fast, unrelated test file. test_helper.exs's boot code
    # (which calls SchemaOwnerGuard.check!/1 then, after migrating,
    # stamp!/1) runs unconditionally as part of loading the file —
    # which target test runs is irrelevant, only that `mix test` boots
    # the suite at all.
    {output, exit_code} =
